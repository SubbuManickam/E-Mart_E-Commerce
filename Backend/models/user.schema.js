...
const uSchema = new mongoose.Schema({
  ...
  password: {
    ...
    validate: {
      validator: (validate) => {
        const reg =
          /^(?=.*[0-9])(?=.*[A-Z])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{10,16}$/;
        return validate.match(reg);
      },
      message: (out) =>
        `Password must have min length of 10 with a special character,number, one capital letter!`,
    },
  },
});
...
module.exports = mongoose.model("userinfo", uSchema);
